...
def up():
    ...
    #Adding Movement limit with pop up question
    moveMent = moveMent - 1
    if moveMent == 0:
        guess1 = wn.numinput("Math Question",  "What is 4-1", None , 0 , 100 )
    if guess1 == 3:
            print("correct")
            moveMent = moveMent + 3 #Why is it not adding three when indented to here?
        #   It is adding, but the turtle is not moving on key inputs. WHy?
        #       I fixed it by putting another wn.listen in the function! BOOYAH BABY!!
            print("count reset")
    else :
        while guess1 != 3:
                guess1 =wn.numinput("Math Question",  "Try again. What is 4-1", None , 0 , 100 )
                    if guess1 == 3:
                        print("correct")
                        moveMent = moveMent + 3 #Why is it not adding three when indented to here?
                    #   It is adding, but the turtle is not moving on key inputs. WHy?
